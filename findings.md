...
## 2026-04-07 Audit Search Findings
- 当前审计页的查询方式仍以“取全量记录再过滤”为主，分页能力缺失。
- 如果继续在内存层做分页，只适合作临时方案，后续仍要重做。
- 基于 Spring Data JPA 直接补分页与组合查询，和当前技术栈最匹配。
- 本批次最合适的切入点是：关键词搜索 + 分页，而不是继续扩展图表或导出。
- 当前实现已完成仓储层组合查询，支持 `keyword + riskLevel + defenseAction + page`。
- 审计页现在可以保留筛选条件翻页，页面交互已经明显更接近完整管理页。
